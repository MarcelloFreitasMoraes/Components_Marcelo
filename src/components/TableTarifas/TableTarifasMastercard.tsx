...
export const TableTarifaMastercard: React.FC<SettlementsProps> = ({
  settlements,
}) => {
  return (
    <S.TableTarifas data-testid="table-tarifas-mastercard">
      <thead>
        <tr>
        ...
        {settlements ? (
          ...
          settlements.map((settlement) => {
            return settlement.taxes?.map((item: TaxesProps) => {
              return item.mastercard?.map((el: CardsProps, index: number) => {
                const funding =
                ...
                return (
                  <tr key={`antecipation-taxes-mastercard-${index}`}>
                    <S.TdStart>
